...
function useHandler(handler) {
    ...
    return useCallback(executeHandler, []);
}
...
function useBackboneGetter(getter, options) {
    ...
    var _e = useState(getResult), result = _e[0], setResult = _e[1];
    var updateResult = function () { return setResult(getResult); };
    useEffect(updateResult, __spreadArray([object], values, true));
    useBackboneEventListener(object, events, updateResult);
    return result;
}

export { useBackboneEventListener, useBackboneGetter };
